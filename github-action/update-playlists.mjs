    ...
    if (fs.existsSync(PATHS.AUTO_UPDATE_LIST)) {
        try {
            ...
            return Object.entries(raw).map(([id, data]) => ({
                ...
                lastKnownRemoteCount: typeof data.lastKnownRemoteCount === 'number' ? data.lastKnownRemoteCount : undefined,
            }));
        } catch (e) {
...
function updateAutoUpdateList(playlistId, title, videoCount, channelId, lastKnownRemoteCount) {
    let list = {};
    if (fs.existsSync(PATHS.AUTO_UPDATE_LIST)) {
        try {
    ...
    if (list[playlistId]) {
        const prev = list[playlistId] || {};
        list[playlistId] = {
            ...prev,
            title,
            videoCount,
            channelId,
            ...(typeof lastKnownRemoteCount === 'number' ? { lastKnownRemoteCount } : {}),
        };
...
async function main() {
    ...
    for (const playlist of localPlaylists) {
        ...
        const baselineCount = typeof playlist.lastKnownRemoteCount === 'number'
            ? playlist.lastKnownRemoteCount
            : playlist.videoCount;
        if (remoteCount !== undefined && remoteCount !== baselineCount) {
            needsUpdate.push({
                id: playlist.id,
                title: playlist.title,
    ...
    for (let i = 0; i < needsUpdate.length; i++) {
        ...
        try {
            ...
            updateAutoUpdateList(
                pl.id,
                updatedPlaylist.title,
                videos.length,
                updatedPlaylist.channelId,
                pl.remoteCount
            );
